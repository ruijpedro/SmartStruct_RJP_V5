...
export default function BuildingModelPage(){
 ...
 <div className="work-grid"><section className="panel"><h3>Edifício</h3><div className="tabs-row"><button className={buildingType==='villa'?'active':''} onClick={()=>{setBuildingType('villa');setFloors(f=>Math.min(f,3))}}>Vivenda · até 3 pisos</button><button className={buildingType==='building'?'active':''} onClick={()=>setBuildingType('building')}>Prédio</button></div><p className="note">A opção Vivenda limita automaticamente o modelo a 3 pisos. Prédio mantém o gerador atual para edifícios de maior desenvolvimento.</p><div className="form-grid"><F l="Pisos" v={floors} s={setFloors}/><F l="Vãos" v={bays} s={setBays}/><F l="Largura vão" u="m" v={bay} s={setBay}/><F l="Pé-direito" u="m" v={storey} s={setStorey}/><F l="Profundidade edifício" u="m" v={depth} s={setDepth}/><F l="q vigas" u="kN/m" v={q} s={setQ}/><F l="Ação horizontal topo" u="kN" v={wind} s={setWind}/><F l="qadm terreno" u="kPa" v={qadm} s={setQadm}/></div></section><section className="panel"><h3>Modelo gerado</h3><Building3DSvg floors={nf} bays={nb} bay={bay} storey={storey} depth={depth} buildingType={buildingType}/>{solved.err&&<div className="solver-error">{solved.err}</div>}</section></div>
...
function Building3DSvg({floors,bays,bay,storey,depth,buildingType}:{floors:number;bays:number;bay:number;storey:number;depth:number;buildingType:'villa'|'building'}){
 const W=700,H=450,scale=Math.min(52,500/Math.max(bays*bay,depth,1),300/Math.max(floors*storey,1));
 const P=(x:number,y:number,z:number)=>({x:W/2+(x-y)*scale*.72,y:H-55-z*scale-(x+y)*scale*.28})
 ...
 return <svg viewBox={`0 0 ${W} ${H}`} className="eng-svg building-3d"><polygon points={ground.map(q=>`${q.x},${q.y}`).join(' ')} className="building3d-ground"/>{slabs}{items}<text x="20" y="25">{buildingType==='villa'?'Vivenda':'Prédio'} · modelo 3D paramétrico</text><text x="20" y="45" className="muted-svg">{floors} pisos · {bays} vãos · profundidade {depth.toFixed(1)} m</text></svg>
}