...
export type FootingInput={N:number;Hx:number;Hy:number;Mx:number;My:number;B:number;L:number;h:number;cb:number;cl:number;cover:number;qAllow:number;mu:number;fck:number;fyk:number;phi:number;offsetX?:number;offsetY?:number}
export function isolatedFootingPro(i:FootingInput){
 const A=i.B*i.L,W=25*A*i.h,Ntot=i.N+W
 // Pilar e sapata são concêntricos por defeito. Se houver desvio geométrico entre eixos,
 // a excentricidade é convertida em momento adicional N·e antes da verificação de contacto.
 const offsetX=i.offsetX||0,offsetY=i.offsetY||0
 const MyEff=i.My+i.N*offsetX,MxEff=i.Mx+i.N*offsetY
 const ex=MyEff/Math.max(Ntot,1e-9),ey=MxEff/Math.max(Ntot,1e-9),kx=Math.abs(ex)<=i.B/6,ky=Math.abs(ey)<=i.L/6
 ...
 const overturnM=Math.hypot(MxEff,MyEff)+H*i.h,restoringM=Ntot*Math.min(i.B,i.L)/2,overturnFS=restoringM/Math.max(overturnM,1e-9)
 ...
 return {A,W,Ntot,offsetX,offsetY,MxEff,MyEff,ex,ey,kx,ky,q0,qmax,qmin,bearingUtil,H,slideResistance,slideFS,overturnM,restoringM,overturnFS,d,u,Vpun,vEd,vRd,punchOK,MxStrip,MyStrip,AsMin,Asx,Asy,spx,spy,ok:bearingUtil<=1&&qmin>=0&&slideFS>=1.5&&overturnFS>=1.5&&punchOK}
}
