...
export type BIMElementType='beam'|'column'|'slab'|'isolated_footing'|'strip_footing'|'raft_foundation'|'pile'|'wall'|'lsf_member'|'pipe'|'manhole'|'road_element'|'furniture'|'plumbing_fixture'|'masonry_unit'|'door'|'window'|'material'
...
export function buildConceptualBuildingBIM(i:BuildingBIMInput):SmartStructBIMModel{
 ...
 for(let r=0;r<rows.length;r++)for(let x=0;x<=i.bays;x++){
  ...
  const axisX=x*i.bay,axisY=rows[r]
  els.push({id:`F${id}`,name:`Sapata ${x+1}.${r+1}`,discipline:'structures',type:'isolated_footing',level:'Fundação',material:concreteC30,
   geometry:{x:axisX-fB/2,y:axisY-fL/2,z:-fH,width:fB,depth:fL,height:fH,axisX,axisY,columnOffsetX:0,columnOffsetY:0},properties:{basis:'pré-dimensionamento automático',axisAlignment:'Pilar e sapata concêntricos por defeito'},
   calculation:ref?{module:'Modelo de Edifício',status:'preliminary',results:{N_kN:ref.N,M_kNm:ref.M,qmax_kPa:ref.qmax,utilizacao:ref.util,excentricidade_geometrica_x_m:0,excentricidade_geometrica_y_m:0}}:undefined});id++
  for(let fl=0;fl<i.floors;fl++)els.push({id:`C${id}`,name:`Pilar ${x+1}.${r+1}.${fl+1}`,discipline:'structures',type:'column',level:`Piso ${fl+1}`,material:concreteC30,
   geometry:{x:axisX-b/2,y:axisY-h/2,z:fl*i.storey,length:i.storey,b,h,axisX,axisY},properties:{orientation:'vertical',axisAlignment:'Eixo coincidente com o centro da sapata'},
...
export function buildStructuralProjectBIM(i:StructuralProjectBIMInput):SmartStructBIMModel{
 ...
 for(const e of i.elements){
  ...
  els.push({id:`SP-${vertical?'C':'V'}-E${e.id}`,name:`${vertical?'Pilar':'Viga'} E${e.id}`,discipline:'structures',type:vertical?'column':'beam',level,material:concrete,
   geometry:vertical?{x:a.x-.35/2,y:-.45/2,z:Math.min(a.y,b.y),length:L,b:.35,h:.45,axis:'Z',axisX:a.x,axisY:0}:{x:Math.min(a.x,b.x),y:0,z:a.y,length:L,b:.25,h:.50,axis:'X'},
 ...
 for(const f of i.foundations){
  ...
  els.push({id:`SP-F-N${f.node}`,name:`Sapata F${f.id}`,discipline:'structures',type:'isolated_footing',level:'Fundação',material:concrete,
   geometry:{x:n.x-f.B/2,y:-f.L/2,z:-f.h,width:f.B,depth:f.L,height:f.h,axisX:n.x,axisY:0,columnOffsetX:0,columnOffsetY:0},
   properties:{sourceModule:'Structural Project PRO',sourceNodeId:f.node,fyk_MPa:i.project.fyk,recobrimento_m:i.project.cover,armadura:reinf,axisAlignment:'Pilar e sapata concêntricos por defeito'},
